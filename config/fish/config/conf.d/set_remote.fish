...
function set_remote -a pid name
    set process_var (_set_remote_process_var $pid)
    set values_var (string join _ $process_var $name)
    set -U -a $process_var $name
    set -U $values_var $argv[3..]
end

function _set_remote_process_var -a pid
    echo _set_remote_$pid
end

function _set_remote_receive -a type event process_var --on-variable (_set_remote_process_var $fish_pid)
